...
export function Button({ href, children, variant = "primary", small = false, type, disabled, onClick }: ButtonProps) { 
  const className = `button ${variant}${small ? " small" : ""}`;
  const inner = <>{children}{variant === "primary" && <ArrowRight size={16} aria-hidden="true" />}</>;
  if (href) {
    return <Link className={className} href={href}>{inner}</Link>; 
  }
  return <button type={type || "button"} className={className} disabled={disabled} onClick={onClick}>{inner}</button>;
}
